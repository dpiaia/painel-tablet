...
Este arquivo é o **arquivo de decisões**, não um manual. Ele conta *por que*
cada peça é como é: o que foi medido, o que foi tentado e não deu, e o que
custou tempo antes de funcionar.

Se você acabou de chegar, comece pelo [README](README.md) e pela
[INSTALACAO](INSTALACAO.md). Volte aqui quando for **mudar** alguma coisa —
quase toda escolha estranha que você encontrar no código tem um parágrafo aqui
explicando que ela não foi por acaso.
...
Quem escreve o plist é `python3 ferramentas/servico.py`. Em Linux e Windows ele
**imprime** o arquivo em vez de instalar — a diferença entre "isto funciona" e
"isto deveria funcionar", e instalar calado um serviço que ninguém verificou
deixa a pessoa com um processo fantasma e nenhuma pista.
